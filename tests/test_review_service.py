...
class TestReviewService:
    ...
    async def test_create_review(self, db_session, seed_data):
        ...
        summary = await ReviewService.get_rating_summary(1)
    ...
    async def test_create_review_no_text(self, db_session, seed_data):
        ...
        reviews = await ReviewService.get_product_reviews(1)
        assert len(reviews) == 1
        assert reviews[0]["rating"] == 4
        assert reviews[0]["text"] is None

    async def test_update_review(self, db_session, seed_data):
        await ReviewService.create_or_update(1, 100, 5, "Супер")
        await ReviewService.create_or_update(1, 100, 3, "Передумал")
    ...
    async def test_multiple_reviews_avg(self, db_session, seed_data):
        await ReviewService.create_or_update(1, 100, 5)
        await ReviewService.create_or_update(1, 101, 4)
        await ReviewService.create_or_update(1, 102, 3)
    ...
    async def test_reviews_isolated_per_product(self, db_session, seed_data):
        await ReviewService.create_or_update(1, 100, 5)
        await ReviewService.create_or_update(3, 100, 2)

        s1 = await ReviewService.get_rating_summary(1)
        s3 = await ReviewService.get_rating_summary(3)
