...
export default function SubscriptionsPage() {
  ...
  function openExtend(sub: PlatformSubscription) {
    setExtendShop(sub);
    setAddDays("30");
  }
  ...
  async function handleExtend() {
    ...
    if (!days || days <= 0) {
      toast.error("Введите количество дней");
      return;
    }
    setExtending(true);
    try {
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Управление подписками всех магазинов
        </p>
      </div>
              ...
              <TableBody>
                {subscriptions.map((sub) => (
                  <TableRow key={sub.id}>
                    ...
                    <TableCell>
                      {sub.is_trial ? (
                        <span className="text-muted-foreground">Триал</span>
                      ) : (
                        sub.plan_name
                      )}
                    </TableCell>
                    ...
                    <TableCell>
                      {sub.is_trial ? "—" : formatPrice(sub.plan_price)}
                    </TableCell>
      ...
      <Dialog
        open={extendShop !== null}
        onOpenChange={(open) => { if (!open) setExtendShop(null); }}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Продлить подписку — {extendShop?.shop_name}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <p className="text-sm text-muted-foreground">
              Текущая дата окончания: {formatDate(extendShop?.expires_at ?? null)}
            </p>
